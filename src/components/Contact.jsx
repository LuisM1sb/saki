...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (recaptchaSiteKey && !recaptchaToken) {
      showAlert('error', t('contact.recaptchaError'));
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        showAlert('success', t('contact.successMessage'));
        setFormData({ name: '', email: '', message: '' });
        setRecaptchaToken(null);
        recaptchaRef.current?.reset();
      } else {
        showAlert('error', result.error || t('contact.errorMessage'));
        setRecaptchaToken(null);
        recaptchaRef.current?.reset();
      }
    } catch (error) {
      console.error('Error:', error);
      showAlert('error', t('contact.connectionError'));
      setRecaptchaToken(null);
      recaptchaRef.current?.reset();
    } finally {
                ...
                <div className="card">
                  ...
                  <div className="space-y-6">
                    <div className="flex items-start">
                      ...
                      <div>
                        <h4 className="text-lg font-semibold text-light-gray mb-1">{t('contact.emailHeading')}</h4>
                        <p className="text-medium-gray">{t('contact.emailValue')}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start">
                      ...
                      <div>
                        <h4 className="text-lg font-semibold text-light-gray mb-1">{t('contact.hoursHeading')}</h4>
                        <p className="text-medium-gray">{t('contact.hoursValue')}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start">
                      ...
                      <div>
                        <h4 className="text-lg font-semibold text-light-gray mb-1">{t('contact.locationHeading')}</h4>
                        <p className="text-medium-gray">{t('contact.locationValue')}</p>
                        <p className="text-medium-gray">{t('contact.remoteNote')}</p>
                      </div>
                    </div>
                  </div>
                </div>
